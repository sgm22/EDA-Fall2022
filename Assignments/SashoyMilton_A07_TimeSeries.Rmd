...
## As Time Series has a seasonal component, we need to first remove the seasonality then interpolate and readd the time series

Ozone_Concentartion_ts <- ts(GaringerOzone$`Daily Max 8-hour Ozone Concentration`, start = c(2010,1), frequency = 365)
Ozone_Concentartion_ts

##Decompse time series

Ozone_Concentration_decomposed <- stl(Ozone_Concentartion_ts, s.window = "periodic", na.action = na.approx)
plot(Ozone_Concentration_decomposed)

#Remove seasonality

adjust_Ozone <- Ozone_Concentartion_ts - Ozone_Concentration_decomposed$seasonality


```

> Answer: * **Piecewise constant**: also known as a "nearest neighbor" approach. Any missing data are assumed to be equal to the measurement made nearest to that date (could be earlier or later).
