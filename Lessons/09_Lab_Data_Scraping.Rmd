...
```{r}
#Apply the API key
census_api_key("d4ad7874c996c7a83fec0a81466309ddc07d8e22",overwrite = TRUE, install = TRUE)
...
retail.price <- 'td:nth-child(2)'
net.summer.capacity <- 'td:nth-child(3)'
net.generation <- 'td:nth-child(4)'
total.retail.sales <- 'td:nth-child(5)'


  
  #Scrape the data, pull tag directy or pull the names created above
 
states <- the_website %>% html_nodes('td:nth-child(1)') %>% html_text()
retail.price <- the_website %>%   html_nodes('td:nth-child(2)') %>%  html_text()
net.summer.capacity <- the_website %>% html_nodes('td:nth-child(3)') %>% html_text()
 net.generation <- the_website %>% html_nodes('td:nth-child(4)') %>% html_text()
 total.retail.sales <- the_website %>% html_nodes('td:nth-child(5)') %>% html_text()
  
  #Convert to dataframe
 
 energy_data <- data.frame(
   states, retail.price, net.summer.capacity,
   net.generation, total.retail.sales
 )
...
big_df <- map(seq(2015,2018), scape.it) %>% bind_rows()
