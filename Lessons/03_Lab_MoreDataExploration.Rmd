...
matrix1[4, ] # Extracting only row 4
matrix1[ , 3] #Extracting only column 3
matrix1[c(12, 14)] # Extracting the group (12,14)
matrix1[c(12:14)] #Extracting 12 through 14
matrix1[2:4, 1:3] #Extracting rows 2 through 4, and columns 1 through 3)

cells <- c(1, 26, 24, 68)
rnames <- c("R1", "R2")
cnames <- c("C1", "C2") 
matrix4 <- matrix(cells, nrow = 2, ncol = 2, byrow = TRUE,
  dimnames = list(rnames, cnames)) # This breaks the data into two rows, two columns grouping by rows and assign the row names and columns assigned above.
matrix4

# Lists ---- 
list1 <- list(name = "Maria", 
              mynumbers = vector1, 
              mymatrix = matrix1, 
              age = 5.3); list1
list1[[2]]

# Data Frames: Each column must be the same data type and must be the same length.
...
dataframe1 <- data.frame(d,e,f) # Data frame containing vectors
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) # This renames d,e, an f to ID, Color and Passed by overwriting the original 

dataframe1[1:2,] # Extracts rows 1 through 2
dataframe1[c("ID","Passed")] #  Exrrcats the columns associated with names indicated
dataframe1$ID
...
> Answer: Vectors show up as value, Matrices show uo as list of numbers while dataframes show up as a table

Question: In the R chunk below, write "dataframe1\$". Press `tab` after you type the dollar sign. What happens?

> Answer: No matches. This is an error.
...
today <- Sys.Date() # The code sys.date returns today's date
format(today, format = "%B") # The code %B returns the month September. This is just an shortcude for the month September.
#format(today, format = "")
#format(today, format = "")
#format(today, format = "")
...
#Ex_3 - on your own...
str_juneteenth2 <- "June 19 1865"
#Since the format is month-day-year we will use function mdy()
date_juneteenth <- mdy(str_juneteenth)

str_juneteenth2 <- "1865 June 03"
date_juneteenth <- ymd(str_juneteenth2)
```
