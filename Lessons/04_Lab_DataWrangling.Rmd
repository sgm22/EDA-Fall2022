---
title: "Lab 4: Data Wrangling - In Class Practice Exercise"
...
ex1 = filter(NTL.phys.data, year4 == 1999)  ## Treat dates as numeric


# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
ex2 = filter(NTL.phys.data, lakename == "Tuesday Lake" &
...
#Note to filter by sampledate, it is easier to mutate a date column from sample date and the filter by the new mutated date column.
...
Ex3 <- 
  ex2 %>%
  mutate (datmonth = month(sampledate)) %>%
  filter(datmonth == 7)
...
#write.csv(NTL.SashoyM_Inclass, row.names = FALSE, 
#          file ="./Data/Processed/NTL-LTER_Lake_Summaries_SashoyM_Processed.csv")
```
