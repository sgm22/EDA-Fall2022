...
subset_PeterPaul.chem.nutrients = subset(PeterPaul.chem.nutrients, depth ==0) # Surfacedepth only

subset_PeterPaul.chem.nutrients %>%
  ggplot(aes(x=daynum,y=temperature_C)) +
  geom_point(aes(color = year4)) +
                facet_wrap(vars(lakename), nrow=2) +
                ylab("Surface Temperarure")
  
PeterPaul.chem.nutrients %>%
  ggplot(aes(day(sampledate), y=temperature_C))
  geom_line()
